...
export default class Canvas{
  ...
  constructor(){
    makeObservable(this, {areaElement:false, zoomElement:false})
  }
   /**
   * 页面布局详情
   * @param id
   */
    ...
    loadScript (data: ScreenDetail) {
      ...
      data.layers?.forEach((layer) => {
        layer.component?.dependencies?.forEach((v:string) => {
          dependencies.add(v);
        });
      });
      // 加载组件依赖库
    ...
    async reload () {
      if (!this.screenInfo) return;
      return service
        .getScreenDetail(this.screenInfo.id)
        .then((data) => {
          if(data){
            data.layers &&
              data.layers.sort((a, b) => {
